...
RAYGUIAPI void GuiLoadStyleFromMemory(const unsigned char* fileData, int dataSize);
...
RAYGUIAPI int GuiListViewEx(Rectangle bounds, char** text, int count, int* scrollIndex, int* active, int* focus);
...
RAYGUIAPI int GuiTabBarEx(Rectangle bounds, char** text, int count, int* hscroll, int* active, int* focus);
// Tab Bar control, using text entries list and returning focus entry
RAYGUIAPI int GuiMessageBox(Rectangle bounds, const char* title, const char* message, const char* btnText,
                            int* btnActive); // Message Box control, displays a message
RAYGUIAPI int GuiTextInputBox(Rectangle bounds, const char* title, const char* message, char* text, int textSize,
                              const char* btnText, int* btnActive,
...
static unsigned int guiIcons[RAYGUI_ICON_MAX_ICONS * RAYGUI_ICON_DATA_ELEMENTS] = {
    ...
    0x3ff00000, 0x201c2010, 0x21042004, 0x22842384, 0x264426c4, 0x2c242fe4, 0x20043e74, 0x00003ffc,
    // ICON_FILETYPE_FONT
    ...
    0x3ff00000, 0x201c2010, 0x23842004, 0x2bf42a04, 0x2fd42814, 0x21c42054, 0x20042004, 0x00003ffc,
    // ICON_FILETYPE_PYTHON
    0x3ff00000, 0x201c2010, 0x20042004, 0x20042004, 0x22842ee4, 0x28a42e84, 0x20042e64, 0x00003ffc, // ICON_FILETYPE_JS
    0x3ff00000, 0x241c2010, 0x2a8c3104, 0x28242454, 0x2ed42004, 0x2a542a54, 0x20042ed4, 0x00003ffc,
    // ICON_FILETYPE_ICON
...
#define RAYGUI_WINDOWBOX_CLOSEBUTTON_HEIGHT      18
    ...
    Rectangle windowPanel = {
        bounds.x, bounds.y + (float)statusBarHeight - (float)statusBorderWidth, bounds.width,
        bounds.height - (float)statusBarHeight + (float)statusBorderWidth
    };
    Rectangle closeButtonRec = {
        statusBar.x + statusBar.width - (float)statusBorderWidth - RAYGUI_WINDOWBOX_CLOSEBUTTON_HEIGHT - vPadding,
    ...
    *view = (GuiGetStyle(LISTVIEW, SCROLLBAR_SIDE) == SCROLLBAR_LEFT_SIDE)
    ...
    if ((state != STATE_DISABLED) && !guiLocked)
    {
        ...
        if (CheckCollisionPointRec(mousePoint, bounds))
        {
            ...
            if (hasHorizontalScrollBar && (GUI_KEY_DOWN(KEY_LEFT_CONTROL) || GUI_KEY_DOWN(KEY_LEFT_SHIFT))) scrollPos.x
                += scrollDelta * scrollSpeed.x;
...
#define RAYGUI_TEXTBOX_AUTO_CURSOR_DELAY      1        // Frames delay for autocursor movement
        ...
        (wrapMode == TEXT_WRAP_NONE)) // No wrap mode
    {
        ...
        if (editMode)
        {
            ...
            if ((textLength > textBoxCursorIndex) && GUI_KEY_PRESSED(KEY_DELETE) && (GUI_KEY_DOWN(KEY_LEFT_CONTROL) ||
            ...
            else if ((textBoxCursorIndex > 0) && (GUI_KEY_PRESSED(KEY_BACKSPACE) || (GUI_KEY_DOWN(KEY_BACKSPACE) &&
            ...
            if ((textBoxCursorIndex > 0) && GUI_KEY_PRESSED(KEY_LEFT) && (GUI_KEY_DOWN(KEY_LEFT_CONTROL) ||
                GUI_KEY_DOWN(KEY_RIGHT_CONTROL)))
            {
                int offset = textBoxCursorIndex;
                //int accCodepointSize = 0;
            ...
            else if ((textBoxCursorIndex > 0) && (GUI_KEY_PRESSED(KEY_LEFT) || (GUI_KEY_DOWN(KEY_LEFT) &&
                autoCursorShouldTrigger)))
            {
                int prevCodepointSize = 0;
                GetCodepointPrevious(text + textBoxCursorIndex, &prevCodepointSize);
            ...
            else if ((textLength > textBoxCursorIndex) && GUI_KEY_PRESSED(KEY_RIGHT) && (GUI_KEY_DOWN(KEY_LEFT_CONTROL)
                || GUI_KEY_DOWN(KEY_RIGHT_CONTROL)))
            {
                int offset = textBoxCursorIndex;
                //int accCodepointSize = 0;
            ...
            else if ((textLength > textBoxCursorIndex) && (GUI_KEY_PRESSED(KEY_RIGHT) || (GUI_KEY_DOWN(KEY_RIGHT) &&
                autoCursorShouldTrigger)))
            {
                int nextCodepointSize = 0;
                GetCodepointNext(text + textBoxCursorIndex, &nextCodepointSize);
...
#define RAYGUI_VALUEBOX_MAX_CHARS  32
    ...
    if ((state != STATE_DISABLED) && !guiLocked && !guiControlExclusiveMode)
    {
        ...
        if (editMode)
        {
            ...
            if ((keyCount >= 0) && (keyCount < RAYGUI_VALUEBOX_MAX_CHARS) && (GuiGetTextWidth(textValue) < bounds.
                width))
            {
    ...
    for (int i = 0; ((i < visibleItems) && (text != NULL)); i++)
    {
        ...
        if (state == STATE_DISABLED)
        {
            ...
            GuiDrawText(text[startIndex + i], GetTextBounds(LISTVIEW, itemBounds),
                        GuiGetStyle(LISTVIEW, TEXT_ALIGNMENT), GetColor(GuiGetStyle(LISTVIEW, TEXT_COLOR_DISABLED)));
        }
        else
        {
            ...
            // && (focus != NULL)) // NOTE: Items focused, despite not returned
            {
                // Draw item focused
                GuiDrawRectangle(itemBounds, GuiGetStyle(LISTVIEW, LIST_ITEMS_BORDER_WIDTH),
...
int GuiTabBarEx(Rectangle bounds, char** text, int count, int* hscroll, int* active, int* focus)
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        if (tabBounds.x < GetScreenWidth())
        {
            ...
            if (CheckCollisionPointRec(GUI_POINTER_POSITION, tabBounds) && GUI_BUTTON_PRESSED_MID) result =
                RESULT_TAB_CLOSE;
            ...
            if (GuiGetStyle(TABBAR, TAB_CLOSE_BUTTON))
            {
                ...
                GuiSetStyle(BUTTON, TEXT_ALIGNMENT, TEXT_ALIGN_CENTER);
#if defined(RAYGUI_NO_ICONS)
                if (GuiButton(RAYGUI_CLITERAL(Rectangle){
                                  tabBounds.x + tabBounds.width - 14 - 5, tabBounds.y + 5, 14, 14
                              }, "x")) result = RESULT_TAB_CLOSE;
#else
                if (GuiButton(RAYGUI_CLITERAL(Rectangle){
                                  tabBounds.x + tabBounds.width - 14 - 5, tabBounds.y + 5, 14, 14
                              },
    ...
    GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle){bounds.x, bounds.y + bounds.height - 1, bounds.width, 1}, 0, BLANK,
                     GetColor(GuiGetStyle(TABBAR, BORDER_COLOR_NORMAL)));
    //--------------------------------------------------------------------
...
int GuiColorPanel(Rectangle bounds, const char* text, Color* color)
{
    ...
    if ((hsv.x != prevHsv.x) || (hsv.y != prevHsv.y) || (hsv.z != prevHsv.z))
    {
        ...
        *color = RAYGUI_CLITERAL(Color){
            (unsigned char)(255.0f * rgb.x), (unsigned char)(255.0f * rgb.y), (unsigned char)(255.0f * rgb.z), color->a
        };
    }
...
int GuiColorBarHue(Rectangle bounds, const char* text, float* hue)
{
    ...
    if (state != STATE_DISABLED)
    {
        ...
        DrawRectangleGradientEx(RAYGUI_CLITERAL(Rectangle){bounds.x, bounds.y, bounds.width, bounds.height / 6.0f},
                                ...
                                Fade(RAYGUI_CLITERAL(Color){255, 255, 0, 255}, guiAlpha),
                                Fade(RAYGUI_CLITERAL(Color){255, 0, 0, 255}, guiAlpha));
        DrawRectangleGradientEx(RAYGUI_CLITERAL(Rectangle){
                                    bounds.x, bounds.y + 1 * (bounds.height / 6.0f), bounds.width, bounds.height / 6.0f
                                },
                                Fade(RAYGUI_CLITERAL(Color){255, 255, 0, 255}, guiAlpha),
                                ...
                                Fade(RAYGUI_CLITERAL(Color){255, 255, 0, 255}, guiAlpha));
        DrawRectangleGradientEx(RAYGUI_CLITERAL(Rectangle){
                                    bounds.x, bounds.y + 2 * (bounds.height / 6.0f), bounds.width, bounds.height / 6.0f
                                },
                                Fade(RAYGUI_CLITERAL(Color){0, 255, 0, 255}, guiAlpha),
                                ...
                                Fade(RAYGUI_CLITERAL(Color){0, 255, 0, 255}, guiAlpha));
        DrawRectangleGradientEx(RAYGUI_CLITERAL(Rectangle){
                                    bounds.x, bounds.y + 3 * (bounds.height / 6.0f), bounds.width, bounds.height / 6.0f
                                },
                                Fade(RAYGUI_CLITERAL(Color){0, 255, 255, 255}, guiAlpha),
                                ...
                                Fade(RAYGUI_CLITERAL(Color){0, 255, 255, 255}, guiAlpha));
        DrawRectangleGradientEx(RAYGUI_CLITERAL(Rectangle){
                                    bounds.x, bounds.y + 4 * (bounds.height / 6.0f), bounds.width, bounds.height / 6.0f
                                },
                                Fade(RAYGUI_CLITERAL(Color){0, 0, 255, 255}, guiAlpha),
                                ...
                                Fade(RAYGUI_CLITERAL(Color){0, 0, 255, 255}, guiAlpha));
        DrawRectangleGradientEx(RAYGUI_CLITERAL(Rectangle){
                                    bounds.x, bounds.y + 5 * (bounds.height / 6.0f), bounds.width, bounds.height / 6.0f
                                },
                                Fade(RAYGUI_CLITERAL(Color){255, 0, 255, 255}, guiAlpha),
                                Fade(RAYGUI_CLITERAL(Color){255, 0, 0, 255}, guiAlpha),
                                Fade(RAYGUI_CLITERAL(Color){255, 0, 0, 255}, guiAlpha),
...
int GuiColorPanelHSV(Rectangle bounds, const char* text, Vector3* colorHsv)
{
    ...
    Color maxHueCol = {
        (unsigned char)(255.0f * rgbHue.x), (unsigned char)(255.0f * rgbHue.y), (unsigned char)(255.0f * rgbHue.z), 255
    };
...
// NOTE: Button pressed is returned through btnActive parameter, 0 for window close button
int GuiTextInputBox(Rectangle bounds, const char* title, const char* message, char* text, int textSize,
                    const char* btnText, int* btnActive, bool* secretViewActive)
{
...
#define RAYGUI_TEXTINPUTBOX_HEIGHT             26
    ...
    if (secretViewActive != NULL)
    {
        ...
        if (GuiTextBox(RAYGUI_CLITERAL(Rectangle){
                       ...
                       ((*secretViewActive == 1) || textEditMode) ? text : stars, textSize,
                       textEditMode) == RESULT_PRESSED)
        ...
        GuiToggle(RAYGUI_CLITERAL(Rectangle){
                  ...
                  (*secretViewActive == 1) ? GuiIconText(ICON_EYE_ON, NULL) : GuiIconText(ICON_EYE_OFF, NULL),
                  secretViewActive);
#endif
    }
    else
...
void GuiLoadStyleDefault(void)
{
    ...
    GuiSetStyle(DEFAULT, TEXT_LINE_SPACING, 12);
    // DEFAULT, pixels between lines, from bottom of first line to top of second
    GuiSetStyle(DEFAULT, TEXT_ALIGNMENT_VERTICAL, TEXT_ALIGN_MIDDLE);
...
void GuiDrawIcon(int iconId, int posX, int posY, int pixelSize, Color color)
{
    if ((guiIconFontOffsetY > 0) && (iconId < RAYGUI_ICON_MAX_FONT_BACKED))
    {
        ...
        Rectangle srcRec = {
            (float)x * (RAYGUI_ICON_SIZE + 2 * RAYGUI_ICON_FONT_ATLAS_PADDING) + RAYGUI_ICON_FONT_ATLAS_PADDING,
            guiIconFontOffsetY + (float)y * (RAYGUI_ICON_SIZE + 2 * RAYGUI_ICON_FONT_ATLAS_PADDING) +
            RAYGUI_ICON_FONT_ATLAS_PADDING,
            RAYGUI_ICON_SIZE, RAYGUI_ICON_SIZE
        };
        Rectangle dstRec = {
            (float)posX, (float)posY, (float)pixelSize * RAYGUI_ICON_SIZE, (float)pixelSize * RAYGUI_ICON_SIZE
        };
...
#define RAYGUI_ICON_TEXT_PADDING   4
    ...
    for (int i = 0; i < lineCount; i++)
    {
        ...
        if (wrapMode == TEXT_WRAP_NONE) posOffsetY += (float)(GuiGetStyle(DEFAULT, TEXT_SIZE) + GuiGetStyle(
            DEFAULT, TEXT_LINE_SPACING));
        else if ((wrapMode == TEXT_WRAP_CHAR) || (wrapMode == TEXT_WRAP_WORD))
...
static void GuiTooltip(Rectangle controlRec)
{
    if (!guiLocked && guiTooltip && (guiTooltipPtr != NULL) && !guiControlExclusiveMode)
    {
        ...
        GuiPanel(RAYGUI_CLITERAL(Rectangle){
                     controlRec.x, controlRec.y + controlRec.height + 4, textSize.x + 16, textSize.y + 8.0f * lineCount
                 }, NULL);
...
static int GuiFontIconBaking(Image* imFont, Font font, Rectangle* whiteRec)
{
    ...
    int reqIconLines = RAYGUI_ICON_MAX_FONT_BACKED / maxIconsPerLine + RAYGUI_ICON_MAX_FONT_BACKED % maxIconsPerLine +
        1; // One extra line
    ...
    int offsetY = ((maxGlyphRecY + (RAYGUI_ICON_SIZE + 2 * RAYGUI_ICON_FONT_ATLAS_PADDING - 1)) /
            (RAYGUI_ICON_SIZE + 2 * RAYGUI_ICON_FONT_ATLAS_PADDING)) * (RAYGUI_ICON_SIZE + 2 *
            RAYGUI_ICON_FONT_ATLAS_PADDING) +
        RAYGUI_ICON_FONT_ATLAS_PADDING;
